    ...
    def __init__(self, root_dir, mode='train', shift_scale=None, filter_ev=True):
        ...
        if shift_scale is None:
            ...
            self.shift_scale = {
                ...
                "loessOther": (np.mean(self.loessOther), np.std(self.loessOther))
            }
        ...
        if self.filter_ev:
            ...
            self.loessOther = self.loessOther[self.y_real > 0.1]
            self.y_real = None

        if shift_scale is None:
            # add car scaling factors only after potentially filtering out.
            self.shift_scale["car"] = (np.mean(self.car), np.std(self.car))
            self.shift_scale["loessCar"] = (np.mean(self.loessCar), np.std(self.loessCar))
