...
parser.add_argument('--iter_max',  type=int, default=100, help="Number of training iterations")
...
parser.add_argument('--lr_gamma',  type=float, default=0.8, help="Anneling factor of lr")
...
if args.k > 1:
    model = GMCVAE(
        nn=args.model, z_dim=args.z, name=model_name, x_dim=24, warmup=(args.warmup==1), var_pen=args.var_pen, k=args.k
    ).to(device)
else:
    model = CVAE(nn=args.model, z_dim=args.z, name=model_name, x_dim=24, warmup=(args.warmup==1), var_pen=args.var_pen).to(device)
